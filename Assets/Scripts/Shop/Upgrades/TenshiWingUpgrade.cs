...
public class TenshiWingUpgrade : ShopUpgrade
{
    ...
    private void Start()
    {
        Global.InvokeLambda(() => 
        {
            ...
            upgradeInfoList = new()
            {
                new Upgrade($"{targetItemName} Tier 1", $"Decrease [{targetItemName}] cost by 10%",          100_000_000_000_000, $"Requires at least {requirementCount[0]} [{targetItemName}]."),
                ...
                new Upgrade($"{targetItemName} Tier 3", $"Decrease [{targetItemName}] cost by 15%",          400_000_000_000_000, $"Requires at least {requirementCount[2]} [{targetItemName}]."),
