    ...
    private void Start()
    {
        Global.InvokeLambda(() => 
        {
            ...
            upgradeInfoList = new()
            {
                ...
                new Upgrade($"{targetItemName} Tier 3", $"Increase [{targetItemName}] tr/s by 4",       10_000, $"Requires at least {requirementCount[2]} [{targetItemName}]."),
                new Upgrade($"{targetItemName} Tier 4", $"Increase [{targetItemName}] tr/s by 50",     200_000, $"Requires at least {requirementCount[3]} [{targetItemName}]."),
                new Upgrade($"{targetItemName} Tier 5", $"Increase [{targetItemName}] tr/s by 100",    500_000, $"Requires at least {requirementCount[4]} [{targetItemName}].")
            };
